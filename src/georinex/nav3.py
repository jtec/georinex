    ...
    with opener(fn) as f:
        ...
        for line in f:
            ...
            if not sv[0] in fields:
                fields[svtypes[-1]] = _fields(line, sv[0])

            svs.append(sv.replace(" ", "0"))
...
def _sparefields(cf: list[str], sys: str, N: int) -> list[str]:
    ...
    if sys == "G":
        ...
        if N == 29:  # only one trailing spare fields
            cf = cf[:-2]
        elif N == 28:  # zero trailing spare fields
            cf = cf[:-3]
        elif N == 27:  # no middle or trailing spare fields
            cf = cf[:22] + cf[23:-3]
    elif sys == "I":
        # No trailing spare fields populated
        if N == 28:
            cf = cf[:28]
        if N == 29:
            # One trailing spare field populated
            cf = cf[:29]
